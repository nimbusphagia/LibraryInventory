...
const editForm = document.querySelector('.edit-author-form');
const authorIdInput = document.getElementById('form-id');
...
authorsContainer.addEventListener('click', (e) => {
  ...
  if (editBtn) {
    ...
    const firstName = parent.querySelector('.first').textContent.trim();
    const lastName = parent.querySelector('.last').textContent.trim();

    editForm.querySelector('input[name="first_name"]').value = firstName;
    editForm.querySelector('input[name="last_name"]').value = lastName;
    authorIdInput.value = editBtn.dataset.authorId;

    [editForm, veil].forEach(toggleVisibility);

  } else if (deleteBtn) {
    // Delete Form toggle, passing id to form
    authorIdInput.value = deleteBtn.dataset.authorId;
